...
class Presentation:
    ...
    def get_puntoDeVenta(self, invoice):
        ...
        """
        self._check_invoice_voucher_name(invoice)
    ...
    def get_numeroComprobante(self, invoice):
        ...
        """
        self._check_invoice_voucher_name(invoice)
        return invoice.voucher_name.split('-')[1][:8]
    ...
    def get_denominacionPartner(self, invoice):
        ...
        """
        return invoice.partner_id.name if invoice.partner_id.name and not invoice.partner_id.name.isspace() \
    ...
    def get_importeTotal(self, invoice):
        ...
        if invoice.voucher_type_id.denomination_id == invoice.env.ref('l10n_ar_afip_tables.account_denomination_d'):
            for line in invoice.invoice_line_ids.filtered(lambda l: l.price_subtotal < 0 and not any(t.is_vat or t.is_exempt for t in l.tax_ids)):
                amount -= line.price_subtotal
        return self.helper.format_amount(amount)
    ...
    def get_impuestoLiquidado(self, tax):
        ...
        """
        return self.helper.format_amount(abs(tax.amount_currency or tax.balance))
...
class PurchasePresentation(Presentation):
    ...
    def _check_invoice_importation_data(self, invoice):
        if not invoice.voucher_name and not invoice.importation_forward_number:
    ...
    def get_despachoImportacion(self, invoice):
        ...
        if invoice.voucher_name:
            ...
            return invoice.importation_forward_number
        else:
            return ''
