...
class TestSoldCheck(common.TransactionCase):
    ...
    def setUp(self):
        ...
        sold_check = self.env['account.sold.check']
        date_today = fields.Date.context_today(sold_check)
        self.third_check = third_check_proxy.new({
            'name': '12345678',
            'bank_id': self.env['res.bank'].search([], limit=1),
            'amount': 1750,
        ...
        self.third_check_journal = journal_proxy.create({
            ...
            'code': 'CHCK',
            'type': 'bank',
            'default_debit_account_id': self.third_check_journal_account.id,
        ...
        self.journal = journal_proxy.create({
            ...
            'code': 'CYP',
            'type': 'bank',
            'default_debit_account_id': self.journal_account.id,
            'default_credit_account_id': self.journal_account.id,
        })
        self.sold_check = sold_check.new({
            'name': self.env['ir.sequence'].next_by_code('account.sold.check.sequence'),
    ...
    def test_negative_amounts(self):
        ...
        with self.assertRaises(ValidationError):
            self.sold_check.constrain_amount()
    ...
    def test_check_currencies(self):
        self.third_check.currency_id = self.env.ref('base.USD')
        self.sold_check.account_third_check_ids = self.third_check | self.third_check_2
        with self.assertRaises(ValidationError):
            self.sold_check.check_currency()

    def test_move(self):
        new_date = self.sold_check.date + timedelta(days=10)
        self.sold_check.date = new_date
    ...
    def test_move_line_vals(self):
        ...
        assert credit_line.account_id == self.env.company.account_third_check_journal_id.default_credit_account_id
    ...
    def test_move_lines_not_account_third_check(self):
        self.env.company.account_third_check_journal_id.default_credit_account_id = None
        with self.assertRaises(ValidationError):
            self.sold_check.post()
    ...
    def test_move_lines_multi_currency(self):
        ...
        self.third_check_2.currency_id = usd
        self.sold_check.account_third_check_ids = self.third_check | self.third_check_2
        self.sold_check.post()
        lines = self.sold_check.move_id.line_ids
        debit_line = lines.filtered(lambda x: x.debit > 0)
