...
class ResCurrencyRateProvider(models.Model):
    ...
    def _compute_name(self):
        selection_values = dict(self._fields['service'].selection)
        for provider in self:
            provider.name = selection_values.get(provider.service),

    @api.depends('active', 'interval_type', 'interval_number')
    def _compute_update_schedule(self):
        selection_values = dict(self._fields['interval_type'].selection)
        for provider in self:
            ...
            provider.update_schedule = _('%(number)s %(type)s') % {
                ...
                'type': selection_values.get(provider.interval_type),
            }
    ...
    def _update(self, date_from, date_to, newest_only=False):
        ...
        for provider in self:
            ...
            if newest_only:
                data = [max(
                    data,
                    key=lambda x: fields.Date.to_date(x[0])
                )]

            for content_date, rates in data:
                timestamp = fields.Date.to_date(content_date)
    ...
    def _schedule_next_run(self):
        self.ensure_one()
        self.last_successful_run = fields.Date.today()
        self.next_run = (
            datetime.combine(
